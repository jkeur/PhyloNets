...
public class DiGraph {
    ...
    public int level;
    public int[] retside; // 0 = this is the left parent of the reticulation, 1 = right parent, 2 = this child is not a reticulation
    ...
    public Vector findCherries() {
        ...
        for (int c = 0; c < outdeg; c++) {
            if (((DiGraph) children.elementAt(c)).outdeg != 0) {
                cher = false;
            }
        }
        if (cher) {
            ...
            for (int c = 0; c < outdeg; c++) {
                Integer edgeNumber = Integer.valueOf(edgenumber[c]);
        ...
        for (int c = 0; c < outdeg; c++) {
            Collection recVec = ((DiGraph) children.elementAt(c)).findCherries();
    ...
    public void remDummyLeaves() {
        ...
        int c = 0;
        while (c < outdeg) {
            if (((DiGraph) children.elementAt(c)).label.intValue() == DUMMY_NUMBER) {
        ...
        for (int d = 0; d < outdeg; d++) {
            ((DiGraph) children.elementAt(d)).remDummyLeaves();
        }
    }
    ...
    public DiGraph postprocess() {
        ...
        visited = true;
        for (int c = 0; c < outdeg; c++) {
            DiGraph child = ((DiGraph) children.elementAt(c));
            ...
            if (child.outdeg == 1 && ((DiGraph) child.children.elementAt(0)).outdeg == 1) {
                // this is an edge between two reticulations
                // point directly to the child of the child
                child.indeg--;
                ((DiGraph) child.children.elementAt(0)).indeg++;
                children.setElementAt(child.children.elementAt(0), c);
                c--;
                continue;
        ...
        for (int c = 0; c < outdeg; c++) {
            ((DiGraph) children.elementAt(c)).postprocess();
        }
        return this;
    }
...
//        G.retnumber = retnumber;  // ?
        ...
        for (int c = 0; c < outdeg; c++) {
            G.children.add(((DiGraph) children.elementAt(c)).cloneDiGraph());
        }
        clone = G;
        return G;
    }
    ...
    public int numberEdges(int[] num) {
        ...
        for (int c = 0; c < outdeg; c++) {
            edgenumber[c] = num[0];
            num[0]++;
            n += ((DiGraph) children.elementAt(c)).numberEdges(num);
            n++;
        }
        return n;
    }
    ...
    public void numberRetEdges(int[] num) {
        ...
        visited = true;
        for (int c = 0; c < outdeg; c++) {
            if (((DiGraph) children.elementAt(c)).indeg > 1) {
                // this child is a reticulation
                if (((DiGraph) children.elementAt(c)).visited) {
                    retside[c] = 1;
                } else {
                    retside[c] = 0;
                    ((DiGraph) children.elementAt(c)).retnumber = num[0];
                    num[0]++;
                }
            } else {
                retside[c] = 2;
            }
            ((DiGraph) children.elementAt(c)).numberRetEdges(num);
        }
    }
    ...
    public Vector updateClustersTree(ClusterSet CS, int tree) {
        ...
        for (int c = 0; c < outdeg; c++) {
            ...
            Vector childCluster = ((DiGraph) children.elementAt(c)).updateClustersTree(CS, tree);
            ...
            if (((DiGraph) children.elementAt(c)).indeg < 2 & CS.contains(childCluster, tree)) {
    ...
    public void switchAllOn() {
        ...
        for (int c = 0; c < outdeg; c++) {
            switched[c] = true;
            ((DiGraph) children.elementAt(c)).switchAllOn();
        }
    }
    ...
    public boolean clusterDisplayed(Vector cluster) {
        ...
        for (int c = 0; c < outdeg; c++) {
            ...
            if (((DiGraph) children.elementAt(c)).clusterDisplayed(cluster)) {
    ...
    public void clearClusters() {
        ...
        for (int c = 0; c < outdeg; c++) {
            ...
            ((DiGraph) children.elementAt(c)).clearClusters();
        }
    }
    ...
    public void switchRet(boolean[] retOn) {
        ...
        for (int c = 0; c < outdeg; c++) {
            if (retside[c] == 0) {
                ...
                switched[c] = retOn[((DiGraph) children.elementAt(c)).retnumber];
            }
            if (retside[c] == 1) {
                // right parent is on the right side of the reticulation
                switched[c] = !retOn[((DiGraph) children.elementAt(c)).retnumber];
            }
            ((DiGraph) children.elementAt(c)).switchRet(retOn);
        }
    }
    ...
    public Vector updateClusters(ClusterSet CS) {
        ...
        for (int c = 0; c < outdeg; c++) {
            ...
            Vector childCluster = ((DiGraph) children.elementAt(c)).updateClusters(CS);
            childCluster = ClusterSet.sortCluster(childCluster);
            ...
            if (((DiGraph) children.elementAt(c)).indeg < 2 & !clusters[c].contains(childCluster) & CS.contains(childCluster)) {
                // store this cluster as one of the clusters displayed by this outgoing edge
                clusters[c].add(childCluster);
            }
            // add the elements to cluster displayed by the incoming edge
        ...
        for (int d = 0; d < outdeg; d++) {
            ...
            if (edgenumber[d] == e1 && edgenumber[d] == e2) {
                ...
                if (edgenumber[d] == e1 || edgenumber[d] == e2) {
                    ...
                    v.outdeg = 2;
                    children.setElementAt(v, d);
                    if (((DiGraph) v.children.elementAt(0)).outdeg > 0) {
                        ((DiGraph) v.children.elementAt(0)).hangBelowEdges(r, e1, e2);
                    }
                }
            }
            if (edgenumber[d] != e2 && edgenumber[d] != e1) {
                ((DiGraph) children.elementAt(d)).hangBelowEdges(r, e1, e2);
            }
        }
    }
    ...
    public void printNode(int[] num) {
        ...
        if (label.intValue() != 0) {
            ...
            String taxon = CassAlgorithm.stringTaxa.elementAt(number - 1);
            ...
            if (CassAlgorithm.PRINT_EXTRA_INFO) {
                ...
                for (int c = 0; c < outdeg; c++) {
                    num[0]++;
                    ((DiGraph) children.elementAt(c)).printNode(num);
                }
            } else {
                number = num[0];
    ...
    public void printArcs(boolean printClusters, boolean colourEdges) {
        for (int c = 0; c < outdeg; c++) {
            ...
            for (int j = 0; j < clusters[c].size(); j++) {
                ...
                if (cluster.size() > 1) {
                    ...
                    for (int i = 0; i < cluster.size(); i++) {
                        ...
                        String taxonString = CassAlgorithm.stringTaxa.elementAt(taxon.intValue() - 1);
            ...
            if (((DiGraph) children.elementAt(c)).indeg > 1 && colourEdges) {
                edgeColour = "red";
            }

//            System.out.println(number + " -> " + ((DiGraph) children.elementAt(c)).number + "[label=\"" + edgeLabel + "\", color=" + edgeColour + "]");
            final int childNum = ((DiGraph) children.elementAt(c)).number;
            String child = childNum < 1000 ? CassAlgorithm.stringTaxa.elementAt(childNum - 1) : Integer.toString(childNum);
            System.out.println(number + " -> " + child + "[label=\"" + edgeLabel + "\", color=" + edgeColour + "]");
        }
        visited = true;
        for (int c = 0; c < outdeg; c++) {
            if (!((DiGraph) children.elementAt(c)).visited) {
                ((DiGraph) children.elementAt(c)).printArcs(printClusters, colourEdges);
            }
        }
    }
    ...
    public void cleanDiGraph() {
        ...
        MAX_RET = 0;
        for (int c = 0; c < outdeg; c++) {
            ((DiGraph) children.elementAt(c)).cleanDiGraph();
        }
    }

    // This function does NOT return the correct reticulation number!
    public int getRetnumber() {
        if (number > 0) {  // Visited before
            return 0;
        }
        number = 1;  // Visited
        retnumber = indeg > 1 ? 1 : 0;
        for (int c = 0; c < outdeg; c++) {
            retnumber += ((DiGraph) children.elementAt(c)).getRetnumber();
        }
        return retnumber;
    }
    ...
    public void resetIndegrees() {
        indeg = 0;
        for (int c = 0; c < children.size(); c++) {
            ((DiGraph) children.elementAt(c)).resetIndegrees();
        }
    }
/*
